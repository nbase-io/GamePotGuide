...
在Assets/Plugin/Android/AndroidManifest.xml文件中修改或者添加此参数。

![](./images/gamepot_unity_03.png)

将从Google Firebase下载的`google-services.json`文件复制到`/Assets/Plugins/Android/`

**File > Build Settings > Build System**菜单中选择Gradle。

![](./images/gamepot_unity_04.png)

##### 如何设置默认环境设并添加Third Party SDK
...
取消所要使用的Third Party注释。

如使用IGAWORKS时，请对以下项目取消注释。
...
| gamepot_adjust_apptoken          | 使用ADJUST时所获取的应用token                          |
| gamepot_adjust_signature         | 使用ADJUST时所获取的Signature                         |
| gamepot_gpg_id                   | 使用Google Play时ID                                    |
...
| gamepot_naver_global_consumerkey | 从Naver Global Cafe SDK获取的Consumer Key     |
| gamepot_naver_global_cafeid      | 从Naver Global Cafe SDK获取的CafeID           |
| gamepot_igaworks_app_key         | 使用IGAWORKS时获取的应用key                        |
| gamepot_igaworks_hash_key        | 使用IGAWORKS时获取的hashkey                      |
...
手动添加需要为每个`drawable`文件夹放入图像。 ([链接](http://romannurik.github.io/AndroidAssetStudio/icons-notification.html#source.type=clipart&source.clipart=ac_unit&source.space.trim=1&source.space.pad=0&name=ic_stat_gamepot_small))通过此链接制作图像时会自动创建每个文件夹的图像。)
...
GAMEPOT介绍多种广告Third Party的接入方法。请把所要使用的广告添加为如下。
...
// 达到每个等级
LevelInfo levelInfo = new LevelInfo("12"); // 等级值
GamePot.tracking(NCommon.AdActions.LEVEL, levelInfo); // 等级 Tracking
